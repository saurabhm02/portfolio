    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          <motion.div
            className="px-4 py-6 md:w-1/2 lg:w-1/3 xl:w-1/4 min-w-1/2 mx-auto relative z-10 bg-gray-800 rounded-lg shadow-lg overflow-y-scroll max-h-[500px] border border-gray-60"
            initial={{ opacity: 0, y: 50 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 50 }}
          >
            <div className="flex items-center justify-between mb-2">
              <h2 className="modal-title text-xl font-bold text-fadeMainTheme">
                {name}
              </h2>
            </div>
            <div className="modal-body flex flex-col items-center">
              <p className="modal-info text-sm text-center text-lightText mt-4">
                {info}
              </p>
              <div className="modal-skills mt-3">
                <h3 className="text-xl font-semibold text-fadeMainTheme text-center">
                  Skills Used:
                </h3>
                <ul className="list-disc list-inside text-base text-cente text-lightText">
